...
open class AutoCompleteTextField: UITextField {
    ...
    @objc func editingChanged() {
        ...
        let recommendDomain: String = writtenDomain.isEmpty
            ? completionValues.first ?? ""
            : suggestDomain(writtenText: writtenDomain) ?? ""
        ...
        autoCompleteLabel.frame = CGRect(
            x: textWidth + textRect(forBounds: bounds).origin.x,
            y: 0,
            width: self.frame.width - textWidth,
            height: frame.height
        )
    }
    ...
    func suggestDomain(writtenText: String) -> String? {
        return completionValues.first { item in
